...
function MonthlyStats(props) {
  ...
  let januaryDistances = [9, 25, 26, 7, 40, 45, 14, 19, 115.48, 204.55, 131];
  let februaryDistances = [0, 0, 6, 6, 0, 17, 47, 0, 143.14, 45.67, 115];
  let marchDistances = [65, 0, 61, 5, 22, 30, 70, 55, 159.41, 51.49, 5];
  let aprilDistances = [53, 70, 70, 73, 33, 55, 59, 129, 76, 63.18, 58];
  let mayDistances = [72, 23, 106, 37, 106, 15, 78, 185, 64.77, 89.6, 172];
  let juneDistances = [224, 134, 131, 28, 56, 19, 80, 111, 225.55, 107.31, 128];
  let julyDistances = [178, 50, 36, 34, 122, 0, 91, 144, 45.35, 170.43, 56];
  let augustDistances = [76, 52, 66, 55, 26, 13, 125, 115, 195.95, 115.32, 16];
  let septemberDistances = [0, 4, 37, 29, 0, 16, 28, 34, 150.99, 51.5, 7.77];
  let octoberDistances = [4, 4, 4, 11, 0, 0, 28, 0, 58.89, 27.8, 18];
  let novemberDistances = [13, 7, 10, 15, 22, 6, 29, 143, 33.3, 18.86, 37];
  let decemberDistances = [12, 0, 0, 4, 0, 0, 38, 88, 0, 26.77, 0];
